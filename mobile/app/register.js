...
// Google "G" logo — official brand colours (same as login.js)
...
// Facebook "f" logo — official brand colour (same as login.js)
...
export default function RegisterScreen() {
  ...
  const onRegister = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const data = await user_register('', email, password);
      await login(data);
    } catch (e) {
  ...
  const onOAuth = async (provider) => {
    try {
      ...
      if (result.type !== 'success') { setError('OAuth was cancelled.'); return; }
      if (result.params?.error) { setError(result.params.error.replaceAll('_', ' ')); return; }
  ...
  return (
    <AppBackground style={styles.screen}>
      ...
      <View style={styles.card}>
        ...
        {/* OAuth buttons — identical style to login.js */}
        <Pressable
        ...
        >
          {oauthLoading === 'google'
            ...
            : <><GoogleLogo size={20} /><Text style={styles.googleText}>Sign up with Google</Text></>}
        </Pressable>
        ...
        >
          {oauthLoading === 'facebook'
            ...
            : <><FacebookLogo size={20} /><Text style={styles.facebookText}>Sign up with Facebook</Text></>}
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  suitTopLeft: {
    ...
    fontSize: 28, color: colors.gold, opacity: 0.12,
  },
  suitBottomRight: {
  ...
  errorText: { color: colors.redSuit, fontFamily: fonts.body, fontSize: 14 },
  ...
  linkText: { ...buttonStyles.secondaryText },
});
